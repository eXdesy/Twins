...
@Component({
  selector: 'app-home-settings-channels-createChannel',
  templateUrl: './createChannel.component.html',
  styleUrl: './createChannel.component.css'
})
export class CreateChannelComponent {
  ...
  onCreateChannel(foto: string, name: string, category: string, description: string, country: string, price: string, link: string) {
		const countryText = (document.getElementById('countrySelect') as HTMLSelectElement).selectedOptions[0].text;
		this.endpointsService.createChannel(foto, name, category, description, countryText, country, price, link, this.token).subscribe(
      response => {
