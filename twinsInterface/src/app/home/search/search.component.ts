...
export class SearchComponent {
  constructor(private switchService: SwitchService, private endpointsService: EndpointsService, private router: Router) { }
	...
	token: string = '';
  selectedType: string = '';
  placeholder: string = 'Search by profile';
  ...
  onSearch(term: string, type: string) {
    ...
    switch (type) {
      case 'profile':
        this.onSearchUsers(term);
        break;
      case 'group':
        this.onSearchGroups(term);
        break;
      case 'channel':
        this.onSearchChannels(term);
        break;
      default:
        this.clear('Unknown search type:')
    }
  }

  onSearchUsers(username: string) {
    this.endpointsService.searchUsers(username, this.token).subscribe(
      response => {
        console.log(response);
        if (response.message) {
          this.complete(response.message)
        } else if (response.profiles && response.profiles.length > 0) {
          this.renderUserData(response.profiles);
        } else {
      ...
      error => {
        if (error.status === 500) {
          ...
          this.clear(error.error.message)
        } else {
  ...
  onSearchGroups(name: string) {
    this.endpointsService.searchGroups(name, this.token).subscribe(
      response => {
        console.log(response);
      ...
      error => {
        if (error.status === 500) {
          ...
          console.log('Error interno del cliente: ' + error.message, true);
          this.clear(error.error.message)
        } else {
          console.log('Error inesperado: ' + error.message, true);
          this.clear(error.error.message)
        }
      },
    );
    ...
    const handlePan = (event: HammerInput, avatarContainer: HTMLElement) => {
        ...
        avatarContainer.style.transform = `translate(${event.deltaX}px, ${event.deltaY}px) rotate(${event.deltaX / 10}deg)`;
    };
    ...
    const handlePanEnd = (event: HammerInput, avatarContainer: HTMLElement) => {
        ...
        if (event.deltaX > 100) {
            ...
            avatarContainer.style.transform = `translate(0px, 0px) rotate(0deg)`;
        }
    };
  ...
  onSearchChannels(name: string) {
    this.endpointsService.searchChannels(name, this.token).subscribe(
      response => {
  ...
  updatePlaceholder(value: string): void {
    this.placeholder = `Search by ${value}`;
  }
}
